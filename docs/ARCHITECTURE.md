...
行の構築の作業バッファ要素数 / 背景スコープの走査回数 / 文字ごとの属性の表（A24）を引くのに
...
**A24. 文字ごとの属性は層に分ける。シェーピングの境界と装飾の境界は別物。** かつては
...
- **inline**: インライン整形文脈ごとに、(a) 空白の畳み込み（A14）→ (b) **シェーピング属性**が
  同じ区間ごとに `TextMeasurer::shape()`（色・letter-spacing・line-height の境界では切らない。A24）
  ...
  ファイルは段の境界で分けてある（A24 の末尾）。(a)〜(c) の結果 `PreparedParagraph` は
...
シェーピングから決めていて、色でいくつに分かれても動かない。A24）。
