...
**A32. zlib の圧縮レベルは入力の一部。それ以外の設定は固定のまま。フィルタ選択は
...
計測（release、i9-14900KF、21 回の中央値。`png::encode` のみ）:

| 入力 | 修正前（level 9） | 修正後 level 9 | 修正後 level 6（新しい既定） |
|---|---:|---:|---:|
| OG カード 1200x630 | 58.2 ms / 73,689 B | 51.0 ms / 73,689 B | **12.7 ms** / 74,733 B |
| OG カード @2x 2400x1260 | 154.6 ms / 158,939 B | 125.3 ms / 158,939 B | **45.7 ms** / 161,517 B |
| 和文の長いページ 800x1320 | 190.1 ms / 361,482 B | 183.9 ms / 361,482 B | **26.9 ms** / 366,026 B |

フィルタ選択の段だけ（level 0 で測ったもの。zlib の無圧縮ブロックの複写ぶん約 6 ms を含む）:
OG カード 19.2 → 11.5 ms、@2x 75.0 → 46.9 ms、長いページ 26.3 → 16.6 ms。
...
- **encode**: シグネチャ + IHDR + IDAT + IEND。color type 6（RGBA）/ 8bit / 非インターレース。
  ...
  **zlib はレベルだけが入力で（0〜9、既定 6。A32）、ストラテジ・windowBits・memLevel は固定**。
...
`validate(options)` は寸法・`scale`・`compression_level`（0〜9。A32）を見る。**オプションの誤りは
