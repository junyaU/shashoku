...
**追記（2026-09-24 夜）: 計算量の実測と対策。**
上の「**残る最悪値**は 2^d で、これは未対応（実測は不要と判断）」は**取り消す**。サーバー用途では
「入れ子を深くしたら指数的に遅くなるが、どこにも上限が書いていない」ままにできない。測って、直して、
上限をテストで固定した。

- **測り方**: `tests/layout/complexity_test.cpp` に深さ d の入力を作るヘルパ（`nest_stretch()`）と、
  回数と時間を出す口（`DISABLED_StretchRelayoutCostTable`）を置いた。出し方は:
  `build/dev/tests/layout/layout_test --gtest_also_run_disabled_tests --gtest_filter='*StretchRelayoutCostTable*'`。
  時間は `std::chrono::steady_clock` の 3 回の中央値（Debug ビルド・このマシン）で、**環境に依るので
  assert しない**。回数（`Counters::layout_block`）は環境に依らないので assert する。
  3 つの形（どれも row の flex を d 段重ね、align-items は既定の stretch）:
  - **(a) 最悪**: 各段 = [背の高いブロック（段ごとに 1 行ずつ高くする）, 次の段]。
    次の段の伸び幅が必ず > 0 になるので、毎段で組み直しが起きる
  - **(b) 鎖**: 各段 = [次の段] だけ。行の交差サイズをその項目自身が決めるので伸び幅 0
  - **(c) A が無い形**: 各段 = [次の段, 1 行のテキスト]。兄弟はいるが背が低いので伸び幅 0

**(a) 最悪の形**（`layout_block` の回数 / ミリ秒）:

| d | 前(A54 なし) | A54 | 対策後 | 前 ms | A54 ms | 対策後 ms |
|---:|---:|---:|---:|---:|---:|---:|
| 1 | 5 | 7 | 7 | 0.072 | 0.071 | 0.072 |
| 2 | 7 | 15 | 11 | 0.136 | 0.147 | 0.150 |
| 3 | 9 | 31 | 15 | 0.207 | 0.263 | 0.230 |
| 4 | 11 | 63 | 19 | 0.316 | 0.470 | 0.353 |
| 5 | 13 | 127 | 23 | 0.445 | 0.826 | 0.483 |
| 6 | 15 | 255 | 27 | 0.599 | 1.451 | 0.677 |
| 7 | 17 | 511 | 31 | 0.746 | 2.613 | 0.884 |
| 8 | 19 | 1023 | 35 | 0.960 | 4.867 | 1.133 |
| 9 | 21 | 2047 | 39 | 1.170 | 9.212 | 1.382 |
| 10 | 23 | 4095 | 43 | 1.454 | 17.844 | 1.652 |
| 11 | 25 | 8191 | 47 | 1.744 | 35.678 | 1.956 |
| 12 | 27 | 16383 | 51 | 2.097 | 68.550 | 2.325 |

**(b) 鎖**:

| d | 前 | A54 | 対策後 | 前 ms | A54 ms | 対策後 ms |
|---:|---:|---:|---:|---:|---:|---:|
| 1 | 4 | 4 | 6 | 0.017 | 0.018 | 0.020 |
| 2 | 5 | 5 | 8 | 0.020 | 0.019 | 0.026 |
| 3 | 6 | 6 | 10 | 0.023 | 0.023 | 0.034 |
| 4 | 7 | 7 | 12 | 0.026 | 0.027 | 0.040 |
| 5 | 8 | 8 | 14 | 0.030 | 0.031 | 0.047 |
| 6 | 9 | 9 | 16 | 0.034 | 0.035 | 0.054 |
| 7 | 10 | 10 | 18 | 0.038 | 0.041 | 0.057 |
| 8 | 11 | 11 | 20 | 0.043 | 0.042 | 0.062 |
| 9 | 12 | 12 | 22 | 0.045 | 0.045 | 0.069 |
| 10 | 13 | 13 | 24 | 0.049 | 0.049 | 0.076 |
| 11 | 14 | 14 | 26 | 0.053 | 0.053 | 0.082 |
| 12 | 15 | 15 | 28 | 0.057 | 0.057 | 0.088 |

**(c) A が無い形**:

| d | 前 | A54 | 対策後 | 前 ms | A54 ms | 対策後 ms |
|---:|---:|---:|---:|---:|---:|---:|
| 1 | 5 | 5 | 7 | 0.053 | 0.054 | 0.064 |
| 2 | 7 | 7 | 11 | 0.089 | 0.090 | 0.103 |
| 3 | 9 | 9 | 15 | 0.127 | 0.127 | 0.153 |
| 4 | 11 | 11 | 19 | 0.174 | 0.172 | 0.212 |
| 5 | 13 | 13 | 23 | 0.225 | 0.226 | 0.273 |
| 6 | 15 | 15 | 27 | 0.287 | 0.283 | 0.338 |
| 7 | 17 | 17 | 31 | 0.369 | 0.360 | 0.420 |
| 8 | 19 | 19 | 35 | 0.512 | 0.425 | 0.536 |
| 9 | 21 | 21 | 39 | 0.547 | 0.535 | 0.608 |
| 10 | 23 | 23 | 43 | 0.627 | 0.613 | 0.683 |
| 11 | 25 | 25 | 47 | 0.728 | 0.692 | 0.804 |
| 12 | 27 | 27 | 51 | 0.849 | 0.794 | 0.913 |

- **A54 が持ち込んだぶんと、以前からあったぶん**: 以前から指数だった部分は**ない**。
  (a) は A54 の前は `2d + 3`（線形）で、A54 が `2^(d+2) − 1` にした（d を 1 増やすと **ちょうど 2 倍**。
  d = 12 で 16,383 回・68ms）。(b)(c) は A54 でも変わっていない（省略条件 2「伸び幅 0 なら組み直さない」が
  効くため）。つまり**指数は A54 が新しく持ち込んだもの**で、A22 / A29 で消したはずの
  「1 段につき 2 回組む」が stretch の経路で復活していた
- **採った対策**: 組み直しの「1 回目」を配置ではなく**計測**にする。stretch で伸びうる項目
  （= `contents_depend_on_cross_size()` が真の項目）は、`layout_items()` で箱を作らず
  `measure_block_size()` で伸ばす前の交差サイズだけ測り（`Item::deferred_cross`）、
  行の交差サイズが決まってから **1 回だけ** `layout_block()` する。
  伸び幅が 0 なら「測ったときと同じ条件（交差サイズ不定）」で組むので、絵は A54 と 1 ビットも変わらない
- **計測の中では組み直さない**（`LayoutEngine::measuring()`）。計測が返すのは箱の**大きさ**だけで、
  中の座標は捨てられる。row の行の交差サイズは組み直しの前に決まり、箱の外形は `line_cross` から
  書き戻すので、**組み直しは自分の箱の大きさを変えない**（= 計測の戻り値を変えない）。
  ここを省かないと、計測のたびに下の段を配置しなおすことになり、全体が d の**二乗**になる
  （実測: この一手が無いと (a) は `d² + 3d + 3` = d = 12 で 183 回。あると `4d + 3` = 51 回）
- **残る上限**: (a) `4d + 3`、(b) `2d + 4`、(c) `4d + 3`。どれも **d に線形**で、A54 の前の
  ちょうど 2 倍以内（「測って 1 回 + 置いて 1 回」なので 2 倍が下限）。
  `tests/layout/complexity_test.cpp` の `StretchRelayoutStaysLinearInTheNestingDepth` が
  d = 1…12 で「1 段あたりの増分 ≤ 6」と「合計 ≤ 6d + 16」を検査する（A54 のままだと d = 2 で落ちる）。
  したがって `RenderLimits` の出番は無い（有限どころか線形で終わる）
- **絵が変わらないことの確認**: `dev` 全件（1288 件）と `asan` 全件が通り、**ゴールデン 16 枚は不変**。
  release バイナリ 3 つ（A54 前 / A54 / 対策後）で 82 枚を描き比べた（依頼集の最終 HTML 25 件を
  `cli_args` そのままで、`docs/guide/examples/*.html` 14 本と `examples/*.html` 5 本を 3 つの幅で）。
  **差が出たのは 2 枚だけ**で、どちらも A54 が直した分（`guided_ja/case02` と `fix/case02`）。
  **対策後は A54 とバイト単位で一致**。4 段の入れ子（矢印つきの図）も目視した
- **実文書での時間**: 依頼集の最終 HTML 25 件を release の CLI で描いた実時間（3 回の中央値の合計）は
  A54 前 933ms / A54 936ms / 対策後 931ms（1 件あたり 37ms）で、**3 つとも区別が付かない**。
...
**A59. 行末に来た全角スペース（U+3000）は行の幅に数えない（ぶら下げる）。実装は
「行分割器にはこのクラスタを SP（U+0020）として渡す」。**（2026-09-25）
...
- **経緯**: 再測定（`docs/benchmark/results_a57_2026-09-25.md` §3）の case10（縦書きの短歌
  ...
  行の幅に数えていた**こと。行末の U+3000 は直前の文字から離せない（LB21 × BA）ので、
  「し + 全角スペース」で溢れると**手前の「し」まで道連れにして**次の行へ行く
- **出典**: CSS Text 3 §4.1.3「Phase II: Trimming and Positioning」の 4。
  「行末に残った white space・**other space separators**・保存されたタブの並びは、
  `white-space` が normal / nowrap なら**無条件でぶら下げる**（hang）」。
  other space separators は同§で「Unicode の general category Zs のうち U+0020 と U+00A0 を
  除いたもの」と定義されていて、**U+3000 は Zs なのでここに入る**（W3C の TR 版で本文を確認した。
  U+3000 を名指しした行は無く、Zs の定義から入る、という読み方）。Chrome の参照画像
  （`docs/benchmark/2026-09-23/images/chrome_ref/case10.png`）も、1 列目の末尾の全角アキを
  見せずに 2 列目を天付きで始めていて、この読みと一致する。
  JLREQ / JIS X 4051 については「行末の全角アキは行末からはみ出させてよい」という趣旨の扱いだが、
  **節番号までは確認できていない**（確信が持てないので出典としては CSS Text 3 を正とする）
- **決めたこと**（横書き・縦書きとも同じ）:
  - 行末に来た U+3000 は、1 つでも連続していても行の幅に数えない
  - 行の途中の U+3000 は今までどおり全角 1 字分の送り。行頭に来たものも幅に数える
    （Phase II の 1 が消すのは**畳み込みの対象になる**空白だけで、U+3000 は対象外）
  - `text-align` の寄せと justify の配分は、ぶら下げた U+3000 を含まない幅で行う
    （= 既存のぶら下げ約物と同じ経路。`Line::width` に入らない）
  - `&nbsp;`（U+00A0）と半角スペースの扱いは変えない
  - **固有寸法にも同じ規則が効く**: 区間の末尾に来た U+3000 は max-content / min-content に
    入らない（`min_content_width()` も `strip_trailing()` を通る）。だから
    「あ + 全角スペース」の flex アイテムは、幅が 2 字分ではなく 1 字分になる。
    行末の半角スペースが前からそうだったのと同じで、CSS でも hang するものは幅に数えない
- **実装**（`layout/inline_paragraph.cpp` の `build_text_items()` だけ。**linebreak は変えていない**）:
  U+3000 のクラスタの `linebreak::Item::cp` に **U+0020 を入れて渡す**。行分割器の
  「行末の空白（SP）は幅に数えず `content_end` から除く」（§3.4 (3)）が、そのままぶら下げの口になる。
  読み替えるのは `cp` だけで、送り・グリフ・文字の対応はそのままなので、行の途中では全角のまま
- **`Item::overhang_after` を使えなかった理由**（最初の案）: 掛け（ルビ）は
  ...
  ぶら下げ（`OverflowPolicy::Burasage` / `Line::hang`）とも二重にならない:
  `try_hang()` は先に `strip_trailing()` を通すので、ぶら下げの判定は**全角スペースを除いた**
  行末の文字（= 句読点）に対して行われ、`Line::hang` は 0 のまま
- **SP として渡すことの副作用**（ここだけは「行末の幅」を超える変更なので記録する）: U+3000 が
  UAX #14 の空白越しの規則（LB14 `OP SP* ×` / LB16 / LB17 / LB18 / LB19）に乗る。
  幅 100px に「あいうえ（　かきく」（16px）を流すと、**前は 1 行目が「あいうえ（」で終わって
  いた**（始め括弧が行末に残る = JLREQ の行末禁則に反する）のが、これからは
  「あいうえ」/「（　かきく」になる。どれも UAX #14 を U+0020 に適用したときの挙動と同じで、
  日本語組版としては同等か改善。テストで仕様として固定した
  （`OpeningBracketBeforeAnIdeographicSpaceDoesNotEndALine`）。
  **他の Zs（U+2000〜U+200A / U+202F など）は読み替えない**:
  U+202F は分割禁止の空白（GL）なので SP にすると後ろで割れてしまう
...
- **行末に来た全角スペース（U+3000）は行の幅に数えない**（A59。CSS Text 3 §4.1.3 Phase II の 4
  「行末に残った white space・other space separators の並びは、`white-space` が normal / nowrap なら
  無条件でぶら下げる」。other space separators = Zs から U+0020 と U+00A0 を除いたもの）。
  1 つでも連続していても数えず、**行の途中と行頭では今までどおり全角 1 字分の送り**。
  横書きも縦書きも同じ。`text-align` の寄せと justify の配分も、ぶら下げたぶんを含まない幅で行う。
  実装は (c) で **U+3000 のクラスタの `linebreak::Item::cp` に U+0020 を入れて渡す**だけ
  （行分割器の「行末の空白（SP）は幅に数えず `content_end` から除く」= §3.4 (3) に乗せる。
  読み替えるのは `cp` だけで送り・グリフ・文字の対応は変えない）。`Item::overhang_after` は
  向きが逆で使えない（A59）。U+00A0 と半角スペース、他の Zs は読み替えない
- **flex**（Phase 6）: 単一行のみ（`flex-wrap` は対応外）。CSS Flexbox §9 のアルゴリズムのうち、
  ...
  **row の stretch で伸ばした交差サイズは definite として扱う**（§9.4 step 11 / §9.8。A54）:
  ...
  「行の交差サイズ − 交差軸のマージン」を used cross size にし、**その値を与えて中身を組む**。
  こうしないと、入れ子の flex の `align-items` / column の `justify-content` / 交差軸の
  `margin: auto` / `flex-grow` が、伸ばす前の高さで解かれたまま残る。対象は
  **アイテム自身が flex コンテナのとき**だけ（ふつうのブロックは definite な高さを自分の矩形に
  しか使わない。block 方向の `%` は未対応なので子には伝わらない）。
  column の交差軸は幅で、組む前に決まっている（`prepare_cross_column()`）ので今までどおり 1 回で組む。

  **順番は「測ってから置く」**（A54 の追記。入れ子の深さ d に対して線形にするため）:
  対象のアイテムは `layout_items()` では箱を作らず、`measure_block_size()`（A29 のメモに乗る）で
  伸ばす前の交差サイズだけ測る。行の交差サイズが決まったら、伸び幅が 0 より大きければ
  definite な交差サイズで、0 なら測ったときと同じ条件で、**1 回だけ**組む。
  先に組んでから伸びたぶんを組み直すと 1 段につき 2 回組むことになり、d 段の入れ子で 2^d になる
  （実測 68ms / 16,383 回 @ d = 12）。
  また、**計測の中では組み直さない**（`LayoutEngine::measuring()`）: 組み直しは箱の大きさを
  変えない（行の交差サイズは組み直しの前に決まり、外形はそこから書き戻す）ので、
  大きさしか要らない計測では省ける。省かないと全体が d の二乗になる。
  対策後の上限は `layout_block` が **4d + 3** 回（`complexity_test.cpp` が d = 1…12 で検査する）
