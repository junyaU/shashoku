...
// 超過は ErrorKind::LimitExceeded。詳細は ARCHITECTURE.md A21。
...
- 信頼できない HTML を受けるときは `RenderOptions::limits` で予算を決める。メモリ不足の扱い（`OutOfMemory` は最善努力で、保証は上限の側）は ARCHITECTURE.md A22
