...
| 中核原則 | 対応範囲を明示し、検出した問題を黙って通さず、修正可能な診断を返す（§3-6）。**設計上の目標と現在の機能は区別する**: 一括報告・はみ出しの警告・JSON 診断・strict は A46 で設計済みだが未実装。現在はエラー 1 件ずつ、警告は豆腐のみ |
