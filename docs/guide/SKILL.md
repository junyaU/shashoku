...
先に知っておく 5 点（ガイド §3-(1) / §3-(3) / §2.2 / §4.2 / §5）:
...
- **「ここでは割らない」と指定する手段は `&nbsp;`（U+00A0）だけ。** 数字と単位は `17&nbsp;ms`、
  数字と助数詞は `2027&nbsp;年` と結ぶ（行分割器がその空白を割らない。見た目はふつうの半角スペース）。
  ただし**守れるのはその空白 1 か所**で、和文は語の中でも折れる（`2027&nbsp;年度` は `年` / `度` に割れうる）。
  **`white-space: nowrap` は対応外**なので語全体を 1 かたまりにはできない。
  等分（`flex: 1 1 0`）の列で短い見出し語が `スタイル付きツ` / `リー` のように折れたら、
  **幅を広げるか語を短くする**（ガイド §5）
