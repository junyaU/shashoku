...
- **句の間の全角スペース（U+3000）は、行末に来ると幅に数えません**（行の外にぶら下がります）。
  上の歌のように句を全角アキで区切るとき、行末に来る 1 個は字数に入れなくて構いません
  （ブラウザと同じ折り返しになります）。行の途中では全角 1 字分の送りです
...
- **全角スペース（U+3000）で句を区切ってよい**です。行末に来たぶんは幅に数えない（行の外に
  ぶら下がる）ので、短歌・俳句の分かち書きがブラウザと同じ位置で折り返します
