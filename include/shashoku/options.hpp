...
struct RenderOptions {
  ...
  // 警告（豆腐・紙面からのはみ出し）を失敗にする（ARCHITECTURE.md A46）。true のとき、描画が終わって
  // 警告が 1 件以上あれば PNG を返さず `RenderFailure` を返す。errors には警告 1 件につき
  // `ErrorKind::WarningAsError` のエラーが入り、元の種類・位置・詳細を保つ。サーバーで
