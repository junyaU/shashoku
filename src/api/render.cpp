...
// 捨てる。`LoadedFonts` / `LoadedImages` を渡す経路（A33）では**利用者が**持つ読み取り専用の
...
// 用意済みの画像に (c) の上限を掛け直す（A33）。prepare() に渡した RenderLimits と
...
//   - `LoadedFonts` / `LoadedImages`: 用意済みの共有資源を参照し、上限を掛け直す（A33）
