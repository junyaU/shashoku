...
// 日本語の文章に出てくるものを漏らさないことが目的で、絵文字は
// Emoji_Presentation = Yes のブロックをまとめて入れている（細かい穴は埋めていない）。
constexpr std::array kWideRanges = std::to_array<Range>({
    ...
    {0x2E9B, 0x2EF3},   {0x2F00, 0x2FD5},  // 康熙部首
    {0x2FF0, 0x2FFB},                      // 漢字構成記述文字
    {0x3000, 0x303E},                      // 　（F）、CJK の約物 、。「」（W）
    {0x3041, 0x3096},                      // ひらがな
    {0x3099, 0x30FF},                      // 濁点・カタカナ
    {0x3105, 0x312F},   {0x3131, 0x318E},   {0x3190, 0x31E3},   {0x31F0, 0x321E},
    {0x3220, 0x3247},   {0x3250, 0x4DBF},                      // CJK 統合漢字拡張 A まで
    {0x4E00, 0xA48C},                                          // CJK 統合漢字・彝文字
    {0xA490, 0xA4C6},   {0xA960, 0xA97C},   {0xAC00, 0xD7A3},  // ハングル音節
    {0xF900, 0xFAFF},                                          // CJK 互換漢字
    {0xFE10, 0xFE19},   {0xFE30, 0xFE52},   {0xFE54, 0xFE66},   {0xFE68, 0xFE6B},
    {0xFF01, 0xFF60},  // 全角英数・全角約物（F）
    {0xFFE0, 0xFFE6},  // ￠￡￥ など（F）
    ...
    {0x1FADF, 0x1FAE9}, {0x1FAF0, 0x1FAF8}, {0x20000, 0x2FFFD},  // CJK 統合漢字拡張 B 以降
    {0x30000, 0x3FFFD},
});
