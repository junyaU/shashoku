...
std::size_t CharStyleTable::intern(const style::ComputedStyle& style, text::Direction direction) {
  ...
  const CharStyle pair{.shaping = shaping_slot->second, .decoration = decoration_slot->second};
  const auto [style_slot, style_added] =
      style_index_.try_emplace(std::pair{pair.shaping, pair.decoration}, styles_.size());
  if (style_added) {
    styles_.push_back(pair);
  }
