...
// ---- #5: flex の入れ子 ---------------------------------------------------------
//
// flex は「測ってから置く」ので、同じ段落に何度も触る。固有寸法の計測と実配置が
// それぞれ別に段落を準備していると、シェーピングが入れ子の深さに比例して増える
// （直す前の実測: column の depth 16 で shape_calls = 65,536）。準備済み段落を
// 共有して消す（A29）。測るのは時間ではなく回数。
