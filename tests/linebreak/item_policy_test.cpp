...
// （issue #2 / ARCHITECTURE.md A19・§3.4 (7)）。
//
// 境界の規則（A19）:
...
TEST(LineBreakItemPolicy, LooseHyphenUsesTheFollowingItem) {
  ...
  // 2 アイテムにまたがる唯一の loose 規則。A19 の取り決めにより、行頭に来る側
...
TEST(LineBreakItemPolicy, BreakAnywhereDoesNotChangeMinContentWidth) {
  ...
  // アイテムごとの指定でも同じにする（A19）。CSS Text 3 の overflow-wrap: anywhere は
