...
TEST(TextVertical, PunctuationGetsVerticalGlyphs) {
  ...
  for (const char32_t cp : {U'。', U'、', U'ー', U'（', U'」'}) {
    ...
    const ShapedText horizontal = shape_ok(shaper, text, horizontal_style());
    const ShapedText vertical = shape_ok(shaper, text, vertical_style());
...
TEST(TextVertical, KanjiKeepsTheSameGlyph) {
  ...
  const ShapedText horizontal = shape_ok(shaper, U"永", horizontal_style());
  const ShapedText vertical = shape_ok(shaper, U"永", vertical_style());
...
TEST(TextVertical, LatinRunsAreLaidSideways) {
  ...
  const ShapedText shaped = shape_ok(shaper, text, vertical_style());
...
TEST(TextVertical, UprightPenSitsOnTheCentreAxis) {
  ...
  const ShapedText shaped = shape_ok(shaper, U"永", vertical_style());
...
TEST(TextVertical, SidewaysBaselineIsShiftedTowardsTheCentreAxis) {
  ...
  const ShapedText shaped = shape_ok(shaper, U"A", vertical_style());
  ASSERT_EQ(shaped.glyphs.size(), 1U);
  ...
  const FontMetrics metrics = metrics_ok(shaper, vertical_style());
...
TEST(TextVertical, FallsBackSidewaysWhenTheFontHasNoVerticalForm) {
  ...
  const ShapedText latin = shape_ok(latin_shaper, std::u32string(1, kLeftQuote), vertical_style());
  ...
  const ShapedText vertical = shape_ok(japanese_shaper, U"「", vertical_style());
  ASSERT_EQ(vertical.glyphs.size(), 1U);
  EXPECT_FALSE(vertical.glyphs[0].sideways);

  // 和文フォントでも vert を持たない “ は横倒しになる。
  const ShapedText quote =
      shape_ok(japanese_shaper, std::u32string(1, kLeftQuote), vertical_style());
...
TEST(TextVertical, IsDeterministic) {
  ...
  const ShapedText first = shape_ok(shaper, text, vertical_style());
  expect_valid_clusters(first, text.size());

  Shaper other(store);
  const ShapedText second = shape_ok(other, text, vertical_style());
